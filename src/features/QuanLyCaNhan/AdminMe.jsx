...
import { useEditAdminMe } from './useEditAdminMe';
...
function AdminMe() {
  ...
  const { editAdmin, isEditing } = useEditAdminMe();
  ...
  return (
    <div className="items-center antialiased">
      ...
      <div className="font-poppins mt-4 flex flex-row gap-4">
        <form
          className="h-2/5 w-full overflow-x-hidden rounded-lg border border-gray-300 shadow-lg"
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-6 px-6 py-4">
            ...
            <div className="flex flex-col gap-2">
              <Label htmlFor="mota">Mô tả vai trò</Label>
              <input
                id="mota"
                className={`rounded-xl ${errorsInfo.mota ? 'ring-red-500' : 'ring-blue-100'} border-1 border-gray-200 px-3 py-1.5 text-[14px] shadow-lg ring-1 ring-blue-100 outline-none invalid:border-2 invalid:border-pink-500 focus:ring-2 focus:ring-blue-300`}
                disabled={!isEdit}
                {...registerInfo('mota')}
              />
              {errorsInfo.mota && (
                <p className="text-xs text-red-500">
                  {errorsInfo.mota.message}
                </p>
              )}
            </div>
          </div>
