...
const ImportSinhVienForm = () => {
  ...
  const onSubmit = async (data) => {
    if (!data.fileBaoCao) return toast.error('Vui lòng chọn file Excel.');

    const formData = new FormData();
    formData.append('file', data.fileBaoCao[0]);
    formData.append('userType', 'sinhvien');
    setUploading(true);
    try {
      ...
      if (res.status === 'success') {
        const sinhVienIdArr = res.created.map((sv) => sv?.sinhVien._id);
        createThongBao({
          message: `${acc.vaiTro} ${user.ten} vừa tạo mới bạn.`,
          sinhViens: sinhVienIdArr,
        });
        queryClient.invalidateQueries(['sinhviens']);
        toast.success(`Đã import thành công ${res.results} sinh viên`);
      } else {
  ...
  const handleDownloadTemplate = () => {
    ...
    window.location.href = `${apiUrl}/public/template/sinhvien/temp-sinh-vien.xlsx`;
  };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="space-y-4">
            <div className="rounded-lg bg-blue-50 p-4 text-sm text-blue-700">
              ...
              <ul className="mt-1 list-disc space-y-1 pl-5">
                ...
                <li>
                  Các trường bắt buộc: Tên, Email, MSSV, Lớp, Ngày sinh, Học
                  lực.
                </li>
                <li>Email phải đúng định dạng và không trùng lặp.</li>
                <li>
                  MSSV phải là số và không trùng lặp. MSSV sẽ được sử dụng làm
                  tên đăng nhập
                </li>
                <li>
          ...
          <div className="flex items-center justify-end space-x-3">
            ...
            <button
              type="submit"
              disabled={uploading}
              className="flex items-center rounded-md bg-blue-500 px-4 py-2 text-sm font-medium text-white shadow-sm transition-colors hover:bg-blue-700 disabled:bg-blue-400"
            >
              {uploading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              {uploading ? 'Đang xử lý...' : 'Import sinh viên'}
            </button>
          </div>
        </form>
            ...
            <div className="p-4">
              <div className="mb-4 flex items-center space-x-2 text-green-600">
                ...
                <p className="font-medium">
                  Đã tạo thành công {result.results} sinh viên
                </p>
              </div>
                            ...
                            <td className="px-4 py-2 text-sm text-amber-700">
                              {err.userData && (
                                <span className="font-mono text-xs">
                                  {JSON.stringify(err.userData).slice(0, 60)}
                                  {JSON.stringify(err.userData).length > 60
                                    ? '...'
                                    : ''}
                                </span>
                              )}
...
export default ImportSinhVienForm;
